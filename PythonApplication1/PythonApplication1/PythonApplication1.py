...
while(True):
    ...
    #画准星
    cv2.line(img, (sightX, sightY - 4), (sightX, sightY - 9), (50, 250, 100), 1) #上
    cv2.line(img, (sightX, sightY + 4), (sightX, sightY + 9), (50, 250, 100), 1) #下
    cv2.line(img, (sightX - 4, sightY), (sightX - 9, sightY), (50, 250, 100), 1) #左
    cv2.line(img, (sightX + 4, sightY), (sightX + 9, sightY), (50, 250, 100), 1) #右
    ...
    contours, hierarchy = cv2.findContours(img2, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)    #获取轮廓
    ...
    if n >= 2:                      #图像中找到两个以上的灯条
        for count in range(0, n):
            ...
            while findCache < n:          #未超界，进行匹配运算
                ...
                if abs(angle[count] - angle[findCache]) < 10 and (1.0 < calcCache < 1.4):  #满足匹配条件
                    ...
                    dis.append(math.sqrt((locX[pairNum] - sightX) ** 2 + (locY[pairNum] - sightY) ** 2))
                    cv2.circle(img, (locX[pairNum], locY[pairNum]), 3, (0, 0, 255), -1)  
                    cv2.circle(img, (locX[pairNum], locY[pairNum]), 8, (0, 0, 255), 2)  #画两个圆来显示中心点的位置
                    print("··Group", pairNum, ":")
        ...
        if pairNum != 0:
            ...
            cv2.line(img, (locX[targetNum], 0), (locX[targetNum], 480), (0, 255, 0), 1)     #画竖线
            cv2.line(img, (0, locY[targetNum]), (720, locY[targetNum]), (0, 255, 0), 1)     #画横线
